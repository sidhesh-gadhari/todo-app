        ...
        <div className="auth-container">
            <div className="glass-card">
                ...
                <form onSubmit={handleSubmit} className="auth-form">
                    ...
                    {pass && (
                        <div className="password-checklist-box">
                            <p className={validations.minLength ? "text-green" : "text-red"}>
                                <span>{validations.minLength ? "✓" : "✕"}</span> Between 6 and 15 characters
                            </p>
                            <p className={validations.hasUpperCase ? "text-green" : "text-red"}>
                                <span>{validations.hasUpperCase ? "✓" : "✕"}</span> At least one uppercase letter (A-Z)
                            </p>
                            <p className={validations.hasLowerCase ? "text-green" : "text-red"}>
                                <span>{validations.hasLowerCase ? "✓" : "✕"}</span> At least one lowercase letter (a-z)
                            </p>
                            <p className={validations.hasNumber ? "text-green" : "text-red"}>
                                <span>{validations.hasNumber ? "✓" : "✕"}</span> At least one number (0-9)
                            </p>
                            <p className={validations.hasUniqueChar ? "text-green" : "text-red"}>
                                <span>{validations.hasUniqueChar ? "✓" : "✕"}</span> At least one symbol (@, $, !, %, *, ?, &, #)
                            </p>
                        </div>
                    )}
                    ...
                    <a
                        href={agreement ? `${import.meta.env.VITE_BACKEND_URL || 'http://localhost:3000/api/v1'}/todo/auth/google` : '#'}
                        className={`google-btn-dark ${!agreement ? "disabled-google-btn" : ""}`}
                        onClick={(e) => {
                            if (!agreement) {
                                e.preventDefault();
